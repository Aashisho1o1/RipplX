...
export function FilingItemCard({ filing }: { filing: FilingItem }) {
  ...
  return <Link className="filing-card" to={`/filings/${filing.accession}${demo ? "?demo=1" : ""}`}>
    <div className="filing-heading"><strong>{filing.ticker}</strong><span className="mono muted">{filing.form} · {filing.filed}</span><SeverityBadge severity={filing.severity} /></div>
