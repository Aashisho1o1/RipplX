...
export function BriefPage() {
  ...
  return <main className="page">
    {sample && <div className="notice neutral">{bootstrap.showcase_source === "sec_cache" ? "Public SEC showcase · cached filings and XBRL facts were refreshed by RipplX before publication. Page views never contact EDGAR. This is not your watchlist." : "Bundled SEC showcase · real public filings run through the production pipeline with recorded model output. The bundled copies are abridged, so offsets and hashes prove the stored excerpt. This is not your watchlist."}</div>}
    {resource.error && <div className="notice">Could not refresh. Showing the last successful brief.</div>}
    <header className="brief-header"><div><p className="page-eyebrow">{sample ? "Public SEC companies" : "Your monitored companies"}</p><h1 className="page-title">Today</h1><p className="page-subtitle">What needs attention now, what deserves review this week, and what stayed routine.</p></div><div className="actions">{!sample && !showOnboarding && trackedTickers.length > 0 && <><button className="button primary" onClick={() => start("sync")}><span aria-hidden="true">↻</span> Sync filings from SEC</button><button className="button" onClick={() => navigate("?panel=analysis")}>Analyze newest filing <span aria-hidden="true">→</span></button></>}{sample && <button className="button" onClick={() => navigate("/today")}>Exit showcase</button>}</div></header>
    ...
    {panel === "analysis" && <Drawer title="Run filing analysis" onClose={closePanel}><AnalysisPanel configured={bootstrap.analysis_configured} tickers={trackedTickers} onAnalyze={(formType, ticker) => { closePanel(); start("analyze", formType, ticker); }} onDemo={() => navigate("/brief?demo=1")} /></Drawer>}
  </main>;
}
