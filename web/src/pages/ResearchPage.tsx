...
export function ResearchPage() {
  ...
  return <main className="page"><header className="research-hero"><p className="page-eyebrow">Before you buy</p><h1>Research a company before you buy.</h1><p>Get a concise company brief, downside lenses, verified financial trends, and exact SEC evidence.</p><form className="ticker-search" onSubmit={research}><label className="sr-only" htmlFor="research-ticker">Ticker symbol</label><input id="research-ticker" className="input mono-input" value={ticker} onChange={event => setTicker(event.target.value.toUpperCase())} placeholder="Enter a ticker, e.g. MSFT" required disabled={loading} /><button className="button primary" disabled={loading}>{loading ? "Working…" : "Research company"}</button></form>{loading && <p className="research-progress" role="status" aria-live="polite">{progress || "Starting research…"} RipplX will open the brief when verified SEC data is ready.</p>}{error && <div className="notice">{error}</div>}<p className="hero-note">No saved watch condition, payment, or brokerage connection required.</p></header>
    <JobProgress job={job} />
