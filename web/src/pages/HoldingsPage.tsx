...
export function HoldingsPage() {
  ...
  const tracked = Array.from(new Map([...rows.watching, ...rows.owned].map(row => [row.ticker, row])).values()).sort((left, right) => left.ticker.localeCompare(right.ticker));
  return <main className="page"><header className="page-header"><h1 className="page-title">Tracked companies</h1><div className="actions"><button className="button primary" onClick={() => navigate("?panel=add")}>Add ticker</button><button className="button" onClick={sync}>Sync filings</button></div></header>{error && <div className="notice">{error}</div>}<JobProgress job={job} />
    ...
    <section className="section"><h2 className="section-kicker">Tickers</h2>{tracked.length ? tracked.map(row => <HoldingRow key={row.ticker} row={row} onRemove={remove} />) : <p className="empty-line">No tickers tracked yet.</p>}</section>
...
function AddTickerForm({ onAdded }: { onAdded: () => void }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); setError(""); try { await api("/api/holdings", { method: "POST", body: JSON.stringify({ ticker }) }); onAdded(); } catch (reason) { setError(reason instanceof ApiError ? reason.message : "Could not add this ticker."); } finally { setSaving(false); } }
