...
function HoldingRow({ row, onRemove }: { row: Holding; onRemove: (ticker: string) => void }) {
  return <div className="holding-row"><div className="holding-row-main"><Link to={`/companies/${row.ticker}`} className="holding-row-main"><div><div className="holding-title"><strong>{row.ticker}</strong><OwnedWatchTag owned={row.owned} />{row.severity && !row.owned && <SeverityBadge severity={row.severity} />}{row.thesis && <span className="tag">THESIS</span>}</div><div className="row-meta">{row.owned ? `${row.shares?.toLocaleString() ?? "—"} shares · $${row.cost_basis?.toFixed(2) ?? "—"} cost · ${row.target_weight_pct ?? "—"}% target` : "company-level filing read"}</div><div className="row-meta">{row.compressed_verified_read ?? "no verified financials yet"}</div></div><span className="row-meta">last: {row.last_filing ?? "—"}</span></Link><button className="button ghost" onClick={() => onRemove(row.ticker)}>Remove</button></div></div>;
}
