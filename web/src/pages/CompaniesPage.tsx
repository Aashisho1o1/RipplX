...
export function CompaniesPage() {
  const location = useLocation(); const navigate = useNavigate(); const params = new URLSearchParams(location.search); const panel = params.get("panel"); const demo = params.get("demo") === "1";
  const load = useCallback((signal: AbortSignal) => api<Companies>(readPath(demo, "companies"), { signal }), [demo]); const resource = useResource(load, [demo]);
  ...
  function closePanel() { navigate(demo ? "/companies?demo=1" : "/companies", { replace: true }); }
  ...
  return <main className="page"><header className="page-header"><div><p className="page-eyebrow">{demo ? "Public SEC showcase" : "Watchlist"}</p><h1 className="page-title">Tracked companies</h1><p className="page-subtitle">{demo ? "Explore the curated companies in this read-only SEC showcase." : "Choose the businesses RipplX should watch across new SEC filings."}</p></div>{!demo && <div className="actions"><button className="button" onClick={sync}><span aria-hidden="true">↻</span> Sync filings</button>{tracked.length > 0 && <button className="button primary" onClick={() => navigate("?panel=add")}>Add ticker <span aria-hidden="true">＋</span></button>}</div>}</header>{error && <div className="notice">{error}</div>}<JobProgress job={job} />
    {!tracked.length ? <section className="empty-invitation company-invitation"><p className="section-kicker">Your front door</p><h2>Follow a company you already understand.</h2><p>Enter a US-listed ticker. RipplX resolves it against the SEC company index before adding it.</p>{!demo && <AddTickerForm onAdded={resource.refresh} introductory={false} />}</section> : <section className="section"><div className="surface-header"><div><span className="section-kicker">{demo ? "Showcase companies" : "Your watchlist"}</span><h2>{tracked.length} {tracked.length === 1 ? "company" : "companies"}</h2></div><span className="surface-meta">Newest 10-K/10-Q/8-K indexed per ticker</span></div><div className="company-list">{tracked.map(row => <CompanyRow key={row.ticker} row={row} onRemove={remove} demo={demo} />)}</div></section>}
    {!demo && panel === "add" && <Drawer title="Add ticker" onClose={closePanel}><AddTickerForm onAdded={() => { closePanel(); resource.refresh(); }} /></Drawer>}
  </main>;
}

function CompanyRow({ row, onRemove, demo }: { row: TrackedCompany; onRemove: (ticker: string) => void; demo: boolean }) {
  return <article className="holding-row"><Link to={`/companies/${row.ticker}${demo ? "?demo=1" : ""}`} className="holding-row-link"><div className="holding-copy"><div className="holding-title"><strong>{row.ticker}</strong><code>CIK {row.cik}</code></div><div className="row-meta">{row.compressed_verified_read ?? "No computed financials yet"}</div></div><div className="holding-last"><span>Newest 10-K/10-Q/8-K</span><strong>{row.newest_supported_filing ?? "—"}</strong></div><span className="row-arrow" aria-hidden="true">→</span></Link>{!demo && <button className="button ghost remove-button" onClick={() => onRemove(row.ticker)}>Remove</button>}</article>;
}
