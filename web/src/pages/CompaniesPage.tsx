...
export function CompaniesPage() {
  ...
  return <main className="page"><header className="page-header"><div><p className="page-eyebrow">Watchlist</p><h1 className="page-title">Tracked companies</h1><p className="page-subtitle">Choose the businesses RipplX should watch across new SEC filings.</p></div><div className="actions"><button className="button" onClick={sync}><span aria-hidden="true">↻</span> Sync filings</button>{tracked.length > 0 && <button className="button primary" onClick={() => navigate("?panel=add")}>Add ticker <span aria-hidden="true">＋</span></button>}</div></header>{error && <div className="notice">{error}</div>}<JobProgress job={job} />
    {!tracked.length ? <section className="empty-invitation company-invitation"><p className="section-kicker">Your front door</p><h2>Follow a company you already understand.</h2><p>Enter a US-listed ticker. RipplX resolves it against the SEC company index before adding it.</p><AddTickerForm onAdded={resource.refresh} introductory={false} /></section> : <section className="section watchlist-section"><div className="surface-header"><div><span className="section-kicker">Your watchlist</span><h2>{tracked.length} {tracked.length === 1 ? "company" : "companies"}</h2></div><span className="surface-meta">Newest filing read per ticker</span></div><div className="company-list">{tracked.map(row => <CompanyRow key={row.ticker} row={row} onRemove={remove} />)}</div></section>}
...
function CompanyRow({ row, onRemove }: { row: TrackedCompany; onRemove: (ticker: string) => void }) {
  return <article className="holding-row"><Link to={`/companies/${row.ticker}`} className="holding-row-link"><div className="holding-copy"><div className="holding-title"><strong>{row.ticker}</strong><code>CIK {row.cik}</code></div><div className="row-meta">{row.compressed_verified_read ?? "No computed financials yet"}</div></div><div className="holding-last"><span>Latest filing</span><strong>{row.last_filing ?? "—"}</strong></div><span className="row-arrow" aria-hidden="true">→</span></Link><button className="button ghost remove-button" onClick={() => onRemove(row.ticker)}>Remove</button></article>;
}
