...
export function CompaniesPage() {
  ...
  return <main className="page"><header className="page-header"><div><p className="page-eyebrow">{demo ? "Sample watchlist" : "Watchlist"}</p><h1 className="page-title">Tracked companies</h1><p className="page-subtitle">{demo ? "Explore the companies included in the read-only sample." : "Choose the businesses RipplX should watch across new SEC filings."}</p></div>{!demo && <div className="actions"><button className="button" onClick={sync}><span aria-hidden="true">↻</span> Sync filings</button>{tracked.length > 0 && <button className="button primary" onClick={() => navigate("?panel=add")}>Add ticker <span aria-hidden="true">＋</span></button>}</div>}</header>{error && <div className="notice">{error}</div>}<JobProgress job={job} />
    {!tracked.length ? <section className="empty-invitation company-invitation"><p className="section-kicker">Your front door</p><h2>Follow a company you already understand.</h2><p>Enter a US-listed ticker. RipplX resolves it against the SEC company index before adding it.</p>{!demo && <AddTickerForm onAdded={resource.refresh} introductory={false} />}</section> : <section className="section"><div className="surface-header"><div><span className="section-kicker">{demo ? "Sample companies" : "Your watchlist"}</span><h2>{tracked.length} {tracked.length === 1 ? "company" : "companies"}</h2></div><span className="surface-meta">Newest 10-K/10-Q/8-K indexed per ticker</span></div><div className="company-list">{tracked.map(row => <CompanyRow key={row.ticker} row={row} onRemove={remove} demo={demo} />)}</div></section>}
