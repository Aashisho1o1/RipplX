...
export function CompanyPage() {
  ...
  const [asOf, setAsOf] = useState(demo ? "2024-08-05" : new Date().toISOString().slice(0, 10)); const [showAll, setShowAll] = useState(false);
  const [job, setJob] = useState<Job | null>(null); const [jobError, setJobError] = useState("");
  const load = useCallback((signal: AbortSignal) => api<Metrics>(`/api/companies/${ticker}/metrics?as_of=${asOf}&show_all=${showAll}&demo=${demo}`, { signal }), [ticker, asOf, showAll, demo]);
  const resource = useResource(load, [ticker, asOf, showAll, demo]);
  useEffect(() => { if (!job || !["queued", "running"].includes(job.state)) return; const timer = window.setInterval(() => api<Job>(`/api/jobs/${job.id}`).then(setJob), 700); return () => window.clearInterval(timer); }, [job]);
  ...
  return <main className="page"><button className="button ghost" onClick={() => navigate(-1)}>← Back</button><header className="page-header"><div className="filing-heading"><h1 className="page-title">{metrics.ticker}</h1><OwnedWatchTag owned={metrics.owned} /></div>{!demo && <div className="actions"><button className="button" onClick={() => start("sync")}>Sync filings</button><button className="button secondary" onClick={() => start("analyze")}>Run analysis</button></div>}</header>{jobError && <div className="notice">{jobError}</div>}<JobProgress job={job} /><p className="metric-caption">Computed by versioned deterministic formulas from SEC XBRL facts (never by the LLM) and traceable to those facts. ✓ = a computed value; — = not applicable or data missing.</p><div className="actions section"><label className="mono muted" htmlFor="as-of">As of</label><input id="as-of" className="input mono-input date-input" type="date" value={asOf} onChange={event => setAsOf(event.target.value)} /><button className="button" aria-pressed={showAll} onClick={() => setShowAll(value => !value)}>{showAll ? "Show starter metrics" : "Show all metrics"}</button></div>{metrics.empty ? <div className="notice neutral">{metrics.before_first_filing ? "No filings or verified numbers existed at this date." : `${metrics.ticker}: ${metrics.empty}`}</div> : <><MetricTable rows={metrics.rows} />{showAll && <p className="helper">Extended catalog — not in the digest</p>}</>}<DisclaimerFooter text={disclaimer} /></main>;
}
