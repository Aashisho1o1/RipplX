...
export interface FilingDetail { filing: FilingItem; what_changed: WhatChanged[]; thesis_impact: ThesisImpact[]; verified_numbers: IssuerMetrics | null; verification: Verification | null; shadow_signal: ShadowSignal | null; insufficient_reason: string | null; disclaimer: string }
...
export interface Job { id: string; kind: "sync" | "analysis"; state: "queued" | "running" | "completed" | "partial" | "failed"; created_at: string; items: { key: string; state: string; message: string; verdict: string | null }[]; error: string | null }
