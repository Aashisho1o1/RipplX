...
export interface FilingItem { accession: string; ticker: string; owned: boolean; form: string; filed: string; severity: Severity; material_items: MaterialItem[]; flags: RedFlag[]; manual_review: boolean }
...
export interface Brief { period: { covered: string; filings_in_window: number; analyzed_filings: number }; portfolio: { owned: string[]; watching: string[] }; answer: string; answer_posture: Posture | null; critical_red_flags: FilingItem[]; what_changed: WhatChanged[]; thesis_impact: ThesisImpact[]; verified_numbers: IssuerMetrics[]; open_questions: string[]; boring_filings: string | null; tracked_but_unanalyzed: boolean; disclaimer: string; sample_data: boolean }
...
export interface FilingDetail { filing: FilingItem; what_changed: WhatChanged[]; thesis_impact: ThesisImpact[]; verified_numbers: IssuerMetrics | null; verification: Verification | null; insufficient_reason: string | null; pipeline: PipelineStage[]; disclaimer: string }
export interface Holding { ticker: string; cik: string; owned: boolean; shares: number | null; cost_basis: number | null; target_weight_pct: number | null; horizon: string | null; thesis: string | null; severity: Severity | null; last_filing: string | null; compressed_verified_read: string | null }
...
export interface Bootstrap { setup_required: boolean; sec_user_agent: string; period: string; model_extract: string; model_reason: string; api_key_configured: boolean; api_key_source: string | null; analysis_configured: boolean }
