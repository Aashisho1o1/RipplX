> **📦 ARCHIVED — historical reference only, not read by the agent by default.**
> This is the original v0.2 build specification, verbatim, preserved for detail the live
> `CLAUDE.md` no longer holds (now that Phases 0–7 are built). The live, actively-maintained
> project-context file is the root `/CLAUDE.md`. If something below conflicts with the
> current code or with root `CLAUDE.md`, the code and root `CLAUDE.md` win.

# finwatch — Open-Source Filing Intelligence for Self-Directed Investors
## CLAUDE.md — Consolidated Build Specification v0.2 (ARCHIVED)
