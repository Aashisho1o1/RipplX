...
13. **Issuer identity comes from SEC, never from a stored row** — registration resolves every
    ...
    recycled, so a local row matching a symbol proves nothing about who files under it today; a
    stored row disagreeing on CIK fails closed rather than tracking the previous owner. Share
    classes collapse to one issuer under an order-independent label, since `companies` is shared
    across users and first-writer-wins would let one participant fix the label everyone sees.
